...
def get_owned_stop(db: Session, stop_id: int, current_user: User) -> TripStop:
    ...
    if not stop or not stop.trip or stop.trip.user_id != current_user.id:
...
def get_owned_activity(db: Session, activity_id: int, current_user: User) -> Activity:
    ...
    if not activity or not activity.stop or not activity.stop.trip or activity.stop.trip.user_id != current_user.id:
