...
export default function ItineraryBuilder() {
  ...
  if (!trip) {
    return (
      <div className="max-w-[1440px] mx-auto px-5 md:px-16 py-20 text-center">
        ...
        <p className="text-sm text-[#46464c] mt-2">This journey doesn't exist in your collection.</p>
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-5 md:px-16 py-10">
      {/* Header */}
      <div className="mb-10">
        <Eyebrow color="text-[#00696d]" className="mb-2">Itinerary Builder</Eyebrow>
        <h1 className="display-headline text-3xl md:text-5xl font-bold text-[#1c1c18]">{trip.title}</h1>
        <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-[#76777d]">
      ...
      {/* Budget Overview Bar */}
      <div className="bg-white rounded-3xl border border-[#e6e3dc] p-6 mb-8">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-semibold text-[#46464c]">Budget Allocation</span>
          <span className="text-xs text-[#76777d]">{Math.round((totalAllocated / (trip.totalBudget || 1)) * 100)}% allocated</span>
        </div>
        <div className="w-full h-3 bg-[#e6e3dc] rounded-full overflow-hidden">
          <div
            className="h-full bg-[#00696d] rounded-full transition-all duration-500"
      ...
      <div className="space-y-5">
        {(trip.sections || []).map((section, idx) => (
          <div key={section.id} className="bg-white rounded-3xl border border-[#e6e3dc] p-6 relative group hover:shadow-md transition-shadow duration-300">
            <div className="flex items-start gap-5">
              {/* Grip + Number */}
              <div className="flex flex-col items-center gap-2 pt-1">
                <GripVertical className="w-4 h-4 text-[#c6c6cc] cursor-grab" />
                <Eyebrow color="text-[#00696d]">Section {String(idx + 1).padStart(2, '0')}</Eyebrow>
              </div>
              ...
              <div className="flex-1 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div>
                    <label className="text-[10px] font-semibold tracking-wider uppercase text-[#76777d] block mb-1">City / Destination</label>
                    <input
                      type="text"
                      value={section.city}
                      onChange={(e) => updateSection(tripId, section.id, { city: e.target.value })}
                      className="w-full px-4 py-2 bg-[#fcf9f3] border border-[#e6e3dc] rounded-full text-sm text-[#1c1c18] focus:outline-none focus:border-[#1c1c18] transition-colors"
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-semibold tracking-wider uppercase text-[#76777d] block mb-1">Dates</label>
                    <input
                      type="text"
                      value={section.dates}
                  ...
                  <div>
                    <label className="text-[10px] font-semibold tracking-wider uppercase text-[#76777d] block mb-1">Budget (₹)</label>
                    <input
                      type="number"
                ...
                <div>
                  <label className="text-[10px] font-semibold tracking-wider uppercase text-[#76777d] block mb-1">Notes</label>
                  <textarea
                    value={section.notes || ''}
                    onChange={(e) => updateSection(tripId, section.id, { notes: e.target.value })}
                    rows={2}
                    placeholder="Key highlights, stay details, travel notes..."
                    className="w-full px-4 py-2.5 bg-[#fcf9f3] border border-[#e6e3dc] rounded-xl text-sm text-[#1c1c18] placeholder-[#76777d] focus:outline-none focus:border-[#1c1c18] transition-colors resize-none"
                  />
                </div>

                {/* Section budget bar */}
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-1.5 bg-[#e6e3dc] rounded-full overflow-hidden">
                    <div className="h-full bg-[#dbc3a8] rounded-full" style={{ width: `${Math.min(100, ((section.allocatedBudget || 0) / (trip.totalBudget || 1)) * 100)}%` }} />
                  </div>
                  <span className="text-[10px] text-[#76777d]">{Math.round(((section.allocatedBudget || 0) / (trip.totalBudget || 1)) * 100)}%</span>
                </div>
              ...
              <button
                ...
                className="p-2 rounded-full hover:bg-red-50 text-[#c6c6cc] hover:text-red-600 transition-colors cursor-pointer opacity-0 group-hover:opacity-100"
      ...
      {/* Add Section Card */}
      <div className="mt-6 border-2 border-dashed border-[#e6e3dc] rounded-3xl p-8 text-center hover:border-[#00696d] transition-colors">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <input
            ...
            className="px-4 py-2.5 bg-white border border-[#e6e3dc] rounded-full text-sm w-full sm:w-64 focus:outline-none focus:border-[#1c1c18]"
          />
